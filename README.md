...
refuses to start. The Hyprland config (`hypr/lib/palette.lua`) has a built-in fallback palette.
...
| Workspace → monitor map | `~/.config/relayout/config.sh` (`WS_HOME`) | Start from `examples/relayout.config.sh`. The copy in `hypr/conf/rules.lua` (`ws_home`) is only a fallback for the very first start. |
...
- **Wallpapers** — thumbnail grid → `setwall`. **Monitors** — what is connected
  and a button to `nwg-displays` (which owns `monitors.lua`).
